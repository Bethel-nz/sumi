...
function generateSumiConfigContent(config) {
    const cleanConfig = { ...config };
    if (cleanConfig.port === undefined)
        delete cleanConfig.port;
    if (cleanConfig.basePath === undefined)
        delete cleanConfig.basePath;
    if (cleanConfig.static?.length === 0)
        delete cleanConfig.static;
    return `
...
function generateServerTsContent() {
    return `
...
const globalOptions = defineOptions(z.object({
    ...
    commands: (gopts) => ({
        ...
        dev: defineCommand({
            ...
            action(entryPoint, options) {
                console.log(`🚀 Starting development server with --hot (${entryPoint})...`);
...
async function initProject(basePath) {
    ...
    if (!fs.existsSync(serverPath)) {
        ...
        console.log(`📄 Created server entry point: ${path.relative(basePath, serverPath)}`);
    }
}
try {
    ...
    if (cfgResult.options?.help) {
        ...
        console.log('  init                 Initialize Sumi in the current directory.');
        console.log('  dev [entry-file]     Start the development server (default: src/server.ts).');
        console.log('\nGlobal Options:');
        console.log('  --help               Show help.');
        console.log('  --version            Show version.');
        process.exit(0);
    }
    const commandAction = cfgResult.command.action;

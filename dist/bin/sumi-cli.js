...
function ensureFavicon(publicDirAbs) {
    ...
    if (!fs.existsSync(publicDirAbs))
        fs.mkdirSync(publicDirAbs, { recursive: true });
...
function generateUserIdRouteContent() {
    return `
import { z } from 'zod';
import { createRoute } from '@bethel-nz/sumi/router';
import { resolver } from 'hono-openapi/zod';
...
function generateSimpleRouteContent() {
    return `
import { z } from 'zod';
import { createRoute } from '@bethel-nz/sumi/router';
import { ValidationContext } from '@bethel-nz/sumi/router';
...
function generatePackageJsonContent(projectName) {
...
function generateReadmeContent(projectName) {
    return `# ${projectName}
...
cli
    ...
    const configPath = path.resolve(process.cwd(), options.config);
    if (!fs.existsSync(configPath)) {
        console.error('❌ Error: sumi.config.ts not found in the root of your project.');
        console.error('💡 Run `sumi init` to create one.');
        process.exit(1);
    }
    try {
    ...
    if (type === 'middleware') {
        ...
        if (fs.existsSync(configPath)) {
            try {
                ...
                console.warn('⚠️  Could not read config, using default routes directory');
            }
        }
        ...
        console.error('❌ Invalid type. Use: sumi generate middleware <name> or sumi generate route <path>');
    }
});
// Helper functions
async function initProject(basePath, force = false) {
    ...
    const middlewareDir = path.join(basePath, responses.middlewareDir || 'middleware');
    ...
    if (!fs.existsSync(indexRoutePath)) {
        ...
        console.log(`📄 Created example route: ${path.relative(basePath, indexRoutePath)}`);
    }
    ...
    if (!fs.existsSync(globalMiddlewarePath)) {
        ...
        console.log(`📄 Created global middleware: ${path.relative(basePath, globalMiddlewarePath)}`);
    }
    ...
    if (!fs.existsSync(authMiddlewarePath)) {
        ...
        console.log(`📄 Created auth middleware: ${path.relative(basePath, authMiddlewarePath)}`);
    }
    ...
    if (!fs.existsSync(middlewareFilePath)) {
        ...
        console.log(`📄 Created example middleware: ${path.relative(basePath, middlewareFilePath)}`);
    }
    // Create other files
    const readmePath = path.join(basePath, 'README.md');
    if (!fs.existsSync(readmePath)) {
        fs.writeFileSync(readmePath, generateReadmeContent(path.basename(basePath)));
...
function generateRouteContent(routePath, methods, isCollection) {
    const cleanPath = routePath.replace(/^\/+|\/+$/g, '');
    const segments = cleanPath.split('/');
    ...
    const paramSchema = params.length > 0
        ? `const paramSchema = z.object({
  ${params
            .map((param) => `${param}: z.string().min(1, '${param.charAt(0).toUpperCase() + param.slice(1)} is required'),`)
            .join('\n  ')}
    ...
    schema: {
      ...
      }).describe('Collection query parameters'),${hasParams ? '\n      param: paramSchema,' : ''}
    },
    openapi: {
    ...
    handler: (c) => {
      const { page, limit, search } = c.req.valid('query');${hasParams
                        ? `
    ...
    handler: (c) => {${hasParams
      ...
      }).describe('${cleanPath} creation request').meta({ ref: 'Create${cleanPath.charAt(0).toUpperCase() + cleanPath.slice(1)}Request' })`;
                return `  ${method}: {
    schema: {${hasParams
                    ? `
      param: paramSchema,`
    ...
    handler: (c) => {${hasParams
      ...
      return c.json({
        id: crypto.randomUUID(),${hasParams
                    ? `
        ${params[0]},`
                    : ''}
        ...data,
        ...
        name: z.string().min(1).optional(),
        description: z.string().optional(),
      }).describe('${cleanPath} update request').meta({ ref: 'Update${cleanPath.charAt(0).toUpperCase() + cleanPath.slice(1)}Request' })`;
                return `  ${method}: {
    schema: {${hasParams
                    ? `
      param: paramSchema,`
    ...
    openapi: {
      ...
      description: '${method === 'put' ? 'Replace' : 'Partially update'} a ${cleanPath.split('/').pop() || cleanPath}',
      tags: ['${cleanPath.split('/')[0] || cleanPath}'],
      responses: {
        200: {
...
async function generateRouteFile(routePath, routesDir, methods) {
    ...
    if (fs.existsSync(fullPath)) {
        const { overwrite } = await prompts({
            type: 'confirm',
            name: 'overwrite',
            message: `File ${path.relative(process.cwd(), fullPath)} already exists. Overwrite?`,
            initial: false,
        });
        if (!overwrite) {
...
async function generateMiddlewareFile(middlewareName, configPath) {
    let middlewareDir = 'middleware'; // default
    // Try to read middleware directory from config
    if (configPath && fs.existsSync(configPath)) {
        try {
            const configModule = await import(`file://${path.resolve(configPath)}?v=${Date.now()}`);
            const config = configModule.default;
            middlewareDir = config.middlewareDir || 'middleware';
        }
        catch (error) {
            console.warn('⚠️  Could not read config, using default middleware directory');
        }
    }
...
`;
    const middlewarePath = path.join(process.cwd(), middlewareDir, `${middlewareName}.ts`);
    ...
    console.log(`🔧 Generated middleware: ${path.relative(process.cwd(), middlewarePath)}`);
    console.log(`💡 Usage: Apply this middleware to routes that need ${middlewareName} functionality`);
}
cli.help();
cli.version('0.1.0');
cli.parse();
